...
function wrapper(plugin_info) {
    ...
    self.addAmapButton = function() {
        ...
        // 拖拽功能，兼容IITC平台
        imgBtn.css('user-select', 'none');
        imgBtn.on('mousedown', function(e) {
            var btn = window.jQuery(this);
            var startX = e.clientX;
            var startY = e.clientY;
            var origLeft = parseInt(btn.css('left'));
            var origTop = parseInt(btn.css('top'));
            var dragging = true;
            window.jQuery(document).on('mousemove.amapbtn', function(ev) {
                if (!dragging) return;
                var dx = ev.clientX - startX;
                var dy = ev.clientY - startY;
                btn.css('left', (origLeft + dx) + 'px');
                btn.css('top', (origTop + dy) + 'px');
            });
            window.jQuery(document).on('mouseup.amapbtn', function() {
                dragging = false;
                window.jQuery(document).off('mousemove.amapbtn mouseup.amapbtn');
                // 记忆位置
        ...
        if(pos){
            try{
                ...
                if(obj.left && obj.top){
                    imgBtn.css('left', obj.left);
                    imgBtn.css('top', obj.top);
                }
            }catch(e){}
        }
